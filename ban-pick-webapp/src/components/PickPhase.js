...
const PickPhase = ({ pickRounds, currentRound = 1, bannedItems, pickedItems: globalPickedItems = [], onComplete }) => {
  ...
  return (
    <div className="pick-phase">
      ...
      <div className="current-turn">
        ...
        <p>Step {currentStepIndex + 1} of {pickRounds[currentRound].length}</p>
      </div>
      ...
      <div className="round-overview">
        ...
        <ol>
          {pickRounds[currentRound].map((step, index) => (
            <li 
              key={index} 
              className={index === currentStepIndex ? 'current' : index < currentStepIndex ? 'completed' : ''}
            >
      ...
      <div className="items-grid">
        {mockItems.map((item, index) => {
          ...
          const isPicked = globalPickedItems.some(pick => pick.item === item) ||
                          pickedItems.some(pick => pick.item === item);
          const isAvailable = !isBanned && !isPicked;
          
          // Tìm team nào đã pick item này
          const pickedBy = globalPickedItems.find(pick => pick.item === item) ||
                          pickedItems.find(pick => pick.item === item);
          
          return (
            <button
              key={index}
              className={`item ${isBanned ? 'banned' : isPicked ? `picked ${pickedBy ? pickedBy.team.toLowerCase() : ''}` : ''}`}
