...
const BanPhase = ({
    ...
    const handleBan = (item) => {
        ...
        const newBannedItems = [
            ...bannedItems,
            { item, team: currentTeam, round: currentRound },
        ];
        ...
        if (
            newBanCount.Blue >= round.countPerTeam &&
            newBanCount.Red >= round.countPerTeam
        ) {
    ...
    const isRoundComplete =
        banCount.Blue >= round.countPerTeam &&
        banCount.Red >= round.countPerTeam;
    ...
    return (
        <div className="ban-phase">
            ...
            <div className="team-info">
                <div
                    className={`team ${currentTeam === "Blue" ? "active" : ""}`}
                >
                    <h3>Blue Team</h3>
                    <p>
                ...
                <div
                    className={`team ${currentTeam === "Red" ? "active" : ""}`}
                >
                    <h3>Red Team</h3>
                    <p>
            ...
            <div className="items-grid">
                {mockItems.map((item, index) => {
                    const isBanned =
                        globalBannedItems.some(
                            (bannedItem) => bannedItem.item === item
                        ) ||
                        bannedItems.some(
                            (bannedItem) => bannedItem.item === item
                        );
                    const isPicked = globalPickedItems.some(
                        (pickedItem) => pickedItem.item === item
                    );
                    const isDisabled = isBanned || isPicked || isRoundComplete;

                    // Tìm team nào đã pick item này
                    const pickedBy = globalPickedItems.find(
                        (pickedItem) => pickedItem.item === item
                    );
                    ...
                    return (
                        <button
                            ...
                            className={`item ${
                                isBanned
                                    ? "banned"
                                    : isPicked
                                    ? `picked ${pickedBy.team.toLowerCase()}`
                                    : ""
                            }`}
                        ...
                        >
                            ...
                            {isPicked && (
                                <span
                                    className={`picked-label ${pickedBy.team.toLowerCase()}`}
                                >
