...
const BanPhase = ({ banRounds, currentRound = 1, bannedItems: globalBannedItems = [], pickedItems: globalPickedItems = [], onComplete }) => {
    ...
    <div className="ban-phase">
      ...
      <div className="team-info">
        <div className={`team ${currentTeam === 'Blue' ? 'active' : ''}`}>
          <h3>Blue Team</h3>
          <p>Bans: {banCount.Blue || 0}/{round.countPerTeam}</p>
        </div>
        <div className={`team ${currentTeam === 'Red' ? 'active' : ''}`}>
          <h3>Red Team</h3>
          <p>Bans: {banCount.Red || 0}/{round.countPerTeam}</p>
        </div>
      </div>
      ...
      <div className="items-grid">
        {mockItems.map((item, index) => {
          const isBanned = globalBannedItems.some(bannedItem => bannedItem.item === item) || 
          ...
          return (
            <button
              key={index}
              className={`item ${isBanned ? 'banned' : isPicked ? `picked ${pickedBy.team.toLowerCase()}` : ''}`}
