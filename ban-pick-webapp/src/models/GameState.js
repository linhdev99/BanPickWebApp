...
export const useGameState = (config) => {
    ...
    const handleBanComplete = useCallback(
        (newBans) => {
            const updatedBannedItems = GameController.updateBannedItems(
                bannedItems,
                newBans
            );
            setBannedItems(updatedBannedItems);

            const nextState = GameController.getNextPhase(
                phase,
                currentRound,
                config
            );
    ...
    const handlePickComplete = useCallback(
        (newPicks) => {
            const updatedPickedItems = GameController.updatePickedItems(
                pickedItems,
                newPicks
            );
